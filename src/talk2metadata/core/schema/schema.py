...
class SchemaDetector:
    ...
    def _detect_foreign_keys(
        ...
        rule_based_fks = self._detect_fks_rule_based(
            tables, table_metadata, target_table
        )
    ...
    def _detect_fks_rule_based(
        ...
        """Rule-based FK detection prioritizing target table (star schema pattern).

        For star schema, all dimension tables should connect to the target/center table.
    ...
    def _detect_fks_with_agent(
        ...
        logger.info(
            ...
            f"(rule-based found {len(rule_based_fks)} FKs as reference)..."
        )
